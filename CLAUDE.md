...
```json
{
  ...
  "body": "Short description (1–3 sentences).",
  "source": "Wikipedia",
  "sourceUrl": "https://...",
  "storySlug": null,
  "imageUrl": null
...
- `month` and `day` are integers or `null` (omit granularity when unknown)
- `type`: `"historia"` (Azul milestones), `"genealogia"` (GEDCOM curiosities), `"descubrimiento"` (Genea Azul discoveries)
- `sourceUrl` must start with `http` to be rendered as a link; otherwise shown as plain text
- `storySlug`: when set, renders a "→ Leer historia" link to `#historias/<slug>`
- `imageUrl`: optional path or URL to a small thumbnail shown in the card (e.g. `"img/timeline/flag-it.svg"`); local images live in `img/timeline/`
- Array must be sorted by year ascending

`genealogia` entries are generated by `generateTimelineJson()` in the `gedcom-analyzer` backend (same pattern as `generateSurnamesJson()`). `historia` and `descubrimiento` entries are curated manually.
