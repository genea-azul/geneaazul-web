...
- `type`: `"historia"` (Azul milestones), `"genealogia"` (GEDCOM processed data), `"curiosidades"` (Interesting facts)
...
`genealogia` entries are generated by `generateTimelineJson()` in the `gedcom-analyzer` backend (same pattern as `generateSurnamesJson()`). `historia` and `curiosidades` entries are curated manually.
